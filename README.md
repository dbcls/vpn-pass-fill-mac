...
* 動作ログは標準出力（上記 status.log）に加えて `~/forti_autofill.log` にも追記されます
  * ※ パスワードだけ版 (forti_pass_fill.py) はログファイル出力なし
...
* macOS のメジャーアップデート後にトークンが入らなくなったら、オートメーション許可がリセットされている可能性大
  ...
  * 許可切れを検知すると🐾が通知を出し、`~/forti_autofill.log` にも記録されます
